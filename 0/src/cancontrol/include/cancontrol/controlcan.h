...
//4.定义阿克曼车的反馈信息。
typedef  struct  _Ackermann_Car_Feedback{
	UINT	Gear;//档位信息
    double  Steering;//方向盘转角
	UINT	Drive_V;//车速
	UINT	Braking;//制动状态信息（预留）
	UINT	Parking;//电子驻车状态
	BYTE	WheelSpd[2];//2个后轮轮速
	BYTE	WheelPulse[2];//2个后轮脉冲数
	BYTE	Mileage[5];//累计里程

}Ackermann_Car_Feedback;
//,*Ackermann_Car_Feedback;

//4.定义阿克曼车的下发信息。
typedef  struct  _Ackermann_Car_Command{
	UINT	Gear;//档位信息
    double  Steering;//方向盘转角
	UINT	Drive_V;//车速
	UINT	Braking;//制动状态信息（预留）
	UINT	Parking;//电子驻车状态
//主要是	Gear、Steering、Drive_V、Parking需要改
}Ackermann_Car_Command;
